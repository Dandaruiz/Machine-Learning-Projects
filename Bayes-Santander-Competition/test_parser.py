...
out = open('../data/test_features.csv', 'w+')

writer = csv.writer(out)

with open('../data/train_ver2_15.csv') as f:
    #f.readline()
    for line in f:
        row = [0] * 24
        ...
        #Parse date (data 1)
        date = sample[0].split('-')
        row[0] = time.mktime(datetime.datetime.strptime(sample[0], "%m/%d/%Y").timetuple())
        ...
        if(sample[6] != ''):
            row[6] = time.mktime(datetime.datetime.strptime(sample[6], "%m/%d/%Y").timetuple())
        ...
        if(sample[10] != ''):
            row[10] = time.mktime(datetime.datetime.strptime(sample[10], "%m/%d/%Y").timetuple())
